            ...
            <div class="search-bar">
                ...
                <div class="flex-box flex-space">
                    ...
                    <el-row style="margin:0 2rem 2rem 0">
                        <el-button type="danger" @click="llllll">清空</el-button>
                    ...
                    <el-table :data="tableData.slice((currentPage-1)*pageSize,currentPage*pageSize)" style="width: 100%">
                        ...
                        <el-table-column prop="operation" label="操作">
                            <template slot-scope="scope">
                                <el-link type="primary" @click="Goonline(scope.row)">{{ scope.row.status | dateFormat1 }}</el-link>
                                <el-link type="primary">编辑</el-link>
                                <el-link type="primary" @click="deleteone(scope.row)">删除</el-link>
                            </template>
                        </el-table-column>
...
export default {
    ...
    methods: { //放方法的地方 
        ...
        search() {
            this.$axios.get('/a/a/article/search', {
                    params: {
                        type: this.value,
                        status: this.valuel,
                        createAt: this.value3,
                        updateAt: this.value3,
        ...
        deleteone(row) {
            ...
            if (r == true) {
                this.$axios.delete('/a/a/u/article/' + row.id) //删除请求
                this.$axios.get("/a/a/article/search") //再次请求更新表格
                    .then(res => {
                        console.log(res)
                        if (res.data.code == 0) {
                            this.tableData = res.data.data.articleList
        ...
        Goonline(row) {
            ...
            if (r == true) {
                this.$axios.put('/a/a/u/article/status/', this.$qs.stringify({
                        id: row.id,
                        status: row.status,
                    }))
                    .then(res => {
